            ...
            <div className="flex flex-col gap-2.5">
              {reports.slice(1).map((r) => (
                ...
                r.status === "critical" ? "bg-red-500" : r.status === "review" ? "bg-amber-500" : "bg-teal-500"
                  ...
                  <div className={`absolute left-0 top-[12%] bottom-[12%] w-[3px] rounded-r-full transition-opacity duration-200 opacity-0 group-hover:opacity-100 ${r.status === "critical"
...
function DetailPage({ report, onBack }) {
  const extractFile = (result) => {
    const match = result?.match(/FILE:(.*)/);
    return match ? match[1].trim() : null;
  };
  const filePath = extractFile(report.note);
  ...
  return (
    <div className="min-h-screen font-sans bg-[#F5F0E8] flex flex-col items-center py-12 px-4 pt-24">
      <div className="w-full max-w-[816px] mb-6 flex justify-between items-center">
        ...
        <div className="flex gap-3">
          {filePath && (
            <button
              onClick={() => downloadOriginal(filePath)}
      ...
      <div ref={reportRef} className="bg-white p-14 shadow-lg mx-auto" style={{ minHeight: "1056px", width: "816px" }}>
        ...
        <div className="mb-10">
          ...
          <div className="border border-slate-200 rounded-2xl p-5 min-h-[100px]">
            <p className="text-sm text-slate-800 whitespace-pre-wrap leading-relaxed">{(report.note || "").replace(/FILE:.*/, "")}</p>
          </div>
        </div>
