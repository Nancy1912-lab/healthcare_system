...
const I = {
    ...
    // ── new icons for gender & blood group ──
...
/* ─────────────────────────────────────────
   PATIENT REGISTER  — gender + blood group added
───────────────────────────────────────── */
    ...
    const handleRegister = async () => {
        try {
            const res = await API.post("/patient/register", {
                ...
                phone: f.phone,
                password: f.pass,
                dob: f.dob,                 // ✅ from input
      gender: f.gender,           // ✅ from dropdown
...
function DoctorReg({ onSwitch, role, setRole }) {
    const specs = ["Cardiology", "Neurology", "Orthopedics", "Pediatrics", "Oncology", "Ophthalmology", "Dentistry", "Pulmonology", "General Medicine", "Dermatology", "Psychiatry", "Radiology"];
    const [f, setF] = useState({ name: "", email: "", phone: "", spec: "", pass: "" });
    const u = k => e => setF(p => ({ ...p, [k]: e.target.value }));
